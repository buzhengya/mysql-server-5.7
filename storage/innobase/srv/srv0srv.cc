...
loop:
	...
	while (srv_shutdown_state == SRV_SHUTDOWN_NONE) {
		...
		srv_master_sleep(); // sleep 1s

		MONITOR_INC(MONITOR_MASTER_THREAD_SLEEP);

		if (srv_check_activity(old_activity_count)) { // 用上一次循环的 activity_count和当前的activity_count对比，判断在上一次循环期间是否有 change，当发生需要关注的chang时，调用srv_inc_activity_count 改变当前的 activity_count
